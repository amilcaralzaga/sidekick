...
### @Repository Map Context Provider – Compact Repo Overview

Provide a compact, deterministic overview of your repo (tree, top files, symbols, hotspots). This uses a local sidecar script (`tools/repomap/repomap.py`) and works offline.
...
The output is bounded (tree size and max counts) and cached by repo root + git HEAD when available. Requires Python 3 on your PATH (or set `pythonPath`).
